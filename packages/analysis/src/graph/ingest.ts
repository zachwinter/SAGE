...
export class KuzuGraphIngestor {
  ...
  private async ingestEntities(entities: GraphEntity[]) {
    console.log(`Ingesting ${entities.length} entities...`);
    let successCount = 0;
    let errorCount = 0;
    ...
    try {
      ...
      console.log(`Successfully ingested ${successCount} entities, ${errorCount} failed`);
    } catch (error: any) {
      await this.conn.query("ROLLBACK;");
      console.error("Failed to ingest entities:", error.message);
      throw error;
    }
  }
  ...
  private async ingestRelationships(relationships: GraphRelationship[]) {
    if (relationships.length === 0) {
      console.warn("No relationships to ingest - this might indicate analysis issues");
      return;
    }
    ...
    try {
      ...
      const preparedStatement = await this.conn.prepare(`
        ...
        CREATE (a)-[:CALLS {
          ...
          isAsync: false,
          metadata: map($metadataKeys, $metadataValues)
        }]->(b)
      `);
      ...
      for (const rel of relationships) {
        if (rel.type === "CALLS") {
          try {
            // Convert metadata object to Kuzu MAP format
            const metadataKeys = Object.keys(rel.metadata);
            const metadataValues = Object.values(rel.metadata).map(v => String(v));
          ...
          console.warn(`Unsupported relationship type: ${rel.type}`);
        }
      }

      await this.conn.query("COMMIT;");
      console.log(`Successfully ingested ${successCount} relationships, ${errorCount} failed`);
    } catch (error: any) {
      await this.conn.query("ROLLBACK;");
      console.error("Failed to ingest relationships:", error.message);
      throw error;
    }
  }
    ...
    try {
      ...
      const preparedStatement = await this.conn.prepare(`
        ...
        SET f.extension = $extension,
            ...
            f.relationshipCount = $relationshipCount
      `);
