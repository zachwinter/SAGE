...
export class PrettyTask {
  ...
  private complete = (m = "") =>
    `${chalk.magenta("└─")}${m ? " " + chalk.magenta(m) : ""}`;
  ...
  logKeyValue(key: string, value: string | number, keyPrefix = "", keySuffix = "") {
    const formattedKey =
      chalk.dim(chalk.gray(keyPrefix)) + key + chalk.dim(chalk.gray(keySuffix));
...
export class PrettyTaskList {
  ...
  async execute(): Promise<any[]> {
    ...
    const timeStr = chalk.magenta(chalk.white(chalk.bold(duration) + " ms"));
    console.log(
      this.complete(`All ${this.tasks.length} tasks completed in ${timeStr}`)
    );
