...
export class RustKuzuClient {
  ...
  async query<T = any>(
    ...
    try {
      ...
      const results = this.parseQueryOutput<T>(result);
   ...
   */
  async ensureDbDirectory(databasePath: string): Promise<void> {
   ...
   */
  async databaseExists(databasePath: string): Promise<boolean> {
    try {
   ...
   */
  async deleteDatabase(databasePath: string): Promise<void> {
    try {
  ...
  private escapeQueryValue(value: any): string {
  ...
  private parseQueryOutput<T>(output: string): T[] {
