...
import { createFirstClassEntity, createSourceFileId, isRelationshipAllowed } from "../index.js";
import { analyzeFiles } from "./analyzer.js";
import { createDependencyEntity, createPackageDependencyRelationships, extractDependencies, findUsedDependencies, loadExternalPackageInfo } from "./dependency-parser.js";
import { createApplicationEntity, createPackageEntity, createProjectEntity, findPackages, identifyApplications, parseProjectInfo } from "./project-parser.js";
 ...
 */
export function analyzeToGraph(files, options = {}) {
    const { debug = false } = options;
    if (debug)
        console.log(line, "Starting graph analysis...");
    ...
    if (debug) {
        ...
        console.log(line, `File analysis complete: ${totalEntities} total entities, ${totalCalls} total calls`);
    }
    ...
    if (projectInfo) {
        ...
        if (options.includeDeps) {
            ...
            console.log(line, ` Added ${externalDeps.length} external dependency entities`);
        }
    }
    // First pass: Create first-class entities, then SourceFile entities with proper counts
    const fileEntities = new Map(); // Track SourceFile entities
    const entityCache = new Map(); // Cache entities by file path for faster lookups
    for (const fileResult of fileResults) {
        const relativePath = relative(process.cwd(), fileResult.filePath);
        // Convert code entities to first-class graph entities
        const fileCodeEntities = [];
    ...
    const skippedRelationships = new Map();
    ...
    for (const fileResult of fileResults) {
        ...
        for (const callExpr of fileResult.callExpressions || []) {
            ...
            if (!toEntity) {
                ...
                toEntity = entities.find(e => e.name === callExpr.callee && e.kind !== "SourceFile");
            }
        ...
        for (const entity of fileResult.entities) {
            ...
            if (entity.type === "export") {
                ...
                if (exportEntity && !("isReExport" in entity && entity.isReExport)) {
                    ...
                    const exportedEntity = cachedFileEntities.find(e => e.name === exportedName && e.kind !== "export" && e.kind !== "import");
                    if (exportedEntity) {
                        // Validate EXPORTS relationship schema
                        if (isRelationshipAllowed("EXPORTS", exportedEntity.kind, exportEntity.kind)) {
                            relationships.push({
                                ...
                                confidence: "high",
                                metadata: {
                                    exportType: ("exportType" in entity ? entity.exportType : "unknown") ||
                                        "unknown",
        ...
        for (const entity of fileResult.entities) {
            if (entity.parentScopeId) {
                ...
                const cachedFileEntities = entityCache.get(relativePath) || [];
                const childEntity = cachedFileEntities.find(e => e.name === entity.name &&
                    e.kind === entity.type &&
                    e.line === entity.line);
                if (parentEntity && childEntity) {
                    // Validate CONTAINS relationship schema
                    if (isRelationshipAllowed("CONTAINS", parentEntity.kind, childEntity.kind)) {
                        relationships.push({
        ...
        if (sourceFileEntity) {
            for (const entity of fileResult.entities) {
                ...
                if (!entity.parentScopeId) {
                    const cachedFileEntities = entityCache.get(relativePath) || [];
                    const codeEntity = cachedFileEntities.find(e => e.name === entity.name &&
                        e.kind === entity.type &&
                        e.line === entity.line);
                    if (codeEntity) {
                        // Validate SourceFile CONTAINS relationship schema
                        if (isRelationshipAllowed("CONTAINS", sourceFileEntity.kind, codeEntity.kind)) {
                            relationships.push({
        ...
        for (const entity of fileResult.entities) {
            if (entity.type === "import" && "module" in entity && entity.module) {
                // Find the import entity we created using cached entities
                const cachedFileEntities = entityCache.get(relativePath) || [];
                const importEntity = cachedFileEntities.find(e => e.name === entity.name && e.kind === "import" && e.line === entity.line);
                if (importEntity) {
                    const moduleSpecifier = entity.module;
                    // Store the module specifier in the import entity for later dependency matching
                    importEntity.moduleSpecifier = moduleSpecifier;
    ...
    if (projectEntity && projectInfo) {
        ...
        for (const appEntity of applicationEntities) {
            relationships.push({
                ...
                confidence: "high",
                metadata: {
                    applicationType: appEntity.applicationType
                }
            });
        }
            ...
            for (const entryPointPath of appInfo.entryPoints) {
                ...
                if (sourceFileEntity) {
                    relationships.push({
                        ...
                        confidence: "high",
                        metadata: {
                            entryType: entryPointPath === resolve(projectRoot, appInfo.main || "")
                                ? "main"
        ...
        if (options.includeDeps) {
            ...
            const depRelationships = createPackageDependencyRelationships(packages, externalDeps, dependencyEntities, packageEntities);
            ...
            console.log(line, ` Created ${depRelationships.length} package dependency relationships`);
            console.log(line, ` Created ${usageRelationships.length} import usage relationships`);
        }
    }
    // Final pass: Update SourceFile relationshipCount efficiently
    // Create a map for fast entity ID to file path lookups
    const entityIdToFilePath = new Map();
    ...
    const fileRelationshipCounts = new Map();
