...
import { extractCallExpressions, extractEntitiesFromAST, extractTypeInformation } from "./parser/ts-ast-extractor.js";
...
export function analyzeFiles(files, options = {}) {
    ...
    if (debug)
        console.log(line, `Analyzing ${files.length} files...`);
    for (const filePath of files) {
        try {
            ...
            if (stat.isDirectory()) {
                if (debug)
                    console.log(line, `Skipping directory: ${filePath}`);
                console.warn(line, `Warning: Skipping directory ${filePath} (expected file)`);
                continue;
            }
            ...
            if (debug) {
                console.log(line, analysis.entities.length, "entities", analysis.callExpressions.length, "calls in", filePath.replace(cwd + "/", ""));
            }
        }
        catch (error) {
            ...
            if (error instanceof Error &&
                ...
                console.warn(line, `Warning: Could not read file ${filePath}: ${error.message}`);
            }
            else {
...
export function analyzeFile(filePath, content, options = {}) {
    ...
    try {
        ...
        console.warn(line, `Warning: TypeScript AST parsing failed for ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        return createEmptyResult();
    }
}
