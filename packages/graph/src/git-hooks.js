...
export class GitDatabaseManager {
    projectRoot;
    debug;
    constructor(projectRoot, debug = false) {
     ...
     */
    async commitDatabase(databasePath, commitHash, message) {
        try {
...
Database: ${basename(databasePath)}`;
     ...
     */
    async checkoutDatabaseAtCommit(databasePath, commitHash) {
        try {
            ...
            return {
                success: false,
                error: err('EIO', `Failed to checkout database at commit ${commitHash}: ${error instanceof Error ? error.message : String(error)}`),
            };
        }
    }
    /**
     ...
     */
    async cleanupTempDatabases(databasePath) {
        try {
            const dbDir = dirname(databasePath);
            const dbName = basename(databasePath);
            const files = await fs.readdir(dbDir);
            // Find temp files matching pattern: dbname.{hash}.tmp
            const tempFiles = files.filter(file => file.startsWith(dbName) && file.includes('.tmp'));
            await Promise.allSettled(tempFiles.map(file => fs.rm(`${dbDir}/${file}`, { force: true })));
     ...
     */
    async isCommitIngested(commitHash) {
        try {
    ...
    async runGitCommand(args, outputFile) {
...
export const gitUtils = {
     ...
     */
    async getCurrentCommit(projectPath) {
