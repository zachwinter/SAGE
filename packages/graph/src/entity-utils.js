 ...
 */
export function getEntityKind(entityType) {
    switch (entityType) {
        case "function":
            return "Function";
        case "class":
 ...
 */
export function createEntityId(entity, filePath) {
 ...
 */
export function createSourceFileId(filePath) {
 ...
 */
export function createFirstClassEntity(entity, filePath) {
...
function extractSuperClass(signature) {
    const match = signature.match(/extends\s+(\w+)/);
    return match ? match[1] : "";
}
function extractInterfaces(signature) {
    const match = signature.match(/implements\s+([^{]+)/);
    if (!match)
        return [];
    return match[1].split(",").map(i => i.trim());
}
function extractVariableType(signature) {
    const match = signature.match(/:\s*([^=]+?)(?:\s*=|$)/);
    return match ? match[1].trim() : "any";
}
function extractDefaultValue(signature) {
    const match = signature.match(/=\s*(.+)$/);
    return match ? match[1].trim() : "";
}
function extractInterfaceExtends(signature) {
    const match = signature.match(/extends\s+([^{]+)/);
    if (!match)
        return [];
    return match[1].split(",").map(i => i.trim());
}
function extractInterfaceProperties(signature) {
    // Simple extraction - would need full AST for accurate parsing
    return [];
}
function extractEnumMembers(signature) {
    const match = signature.match(/{\s*([^}]+)\s*}/);
    if (!match)
        return [];
    return match[1]
...
function extractTypeDefinition(signature) {
    const match = signature.match(/=\s*(.+)$/);
    return match ? match[1].trim() : "";
}
function extractTypeParameters(signature) {
    const match = signature.match(/<([^>]+)>/);
    if (!match)
        return [];
    return match[1].split(",").map(p => p.trim());
}
function extractOriginalImportName(localName, signature) {
...
function extractImportPath(signature) {
    const match = signature.match(/from\s+['"]([^'"]+)['"]/);
    return match ? match[1] : "";
}
function extractOriginalExportName(localName, signature) {
