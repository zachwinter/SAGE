 ...
 */
export function isRelationshipAllowed(relationshipType, fromKind, toKind) {
    const allowedPairs = ALLOWED_RELATIONSHIPS[relationshipType];
    if (!allowedPairs) {
 ...
 */
export function filterValidRelationships(relationships, entities, debug = false) {
    ...
    for (const relationship of relationships) {
        ...
        if (isValid) {
            ...
            skippedCount++;
            if (debug) {
                console.warn(`Skipping ${relationship.type} relationship from ${fromKind} to ${toKind} - not allowed in schema`);
            }
        }
    }
