...
function prepareCallTreeFromFunctions(callGraph: Map<string, Set<string>>) {
  const treeData = [];
...
function prepareCallTreeToFunctions(reverseCallGraph: Map<string, Set<string>>) {
  const treeData = [];
...
function prepareFlatTopologicalSort(sortResult) {
  const { sorted, cycles } = sortResult;

  const entities = sorted.map((entity, index) => ({
    ...entity,
    index: index + 1
  }));
...
function buildDependencyTree(sorted, cycles) {
  ...
  const imports = sorted.filter(e => e.type === "import");
  const exports = sorted.filter(e => e.type === "export");
  const types = sorted.filter(e => e.type === "type" || e.type === "interface");
  const variables = sorted.filter(e => e.type === "variable");
  const functions = sorted.filter(e => e.type === "function");
  const classes = sorted.filter(e => e.type === "class");
